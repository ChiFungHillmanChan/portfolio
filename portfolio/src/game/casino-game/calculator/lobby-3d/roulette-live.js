...
import { betSpots, boardStats, EU_WHEEL } from './roulette-map.js';
...
export function openRouletteLive({ table }) {
  ...
  // the 2D game's own stats semantics (boardStats). The last session's
  // records stay on the board after leaving; sitting down again resets it.
  const sessionSpins = [];
  const pushSpin = (n) => {
    sessionSpins.push(n === '00' ? 0 : Number(n) || 0);
    rig.userData.setBoardStats(boardStats(sessionSpins, EU_WHEEL));
  };
  rig.userData.setBoardStats(null);   // fresh board on entry
  ...
  const onMsg = (ev) => {
    ...
    else if (d.type === 'stat-spin') pushSpin(d.result);   // Skip-100 simulated history
  };
