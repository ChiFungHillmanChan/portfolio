(() => {
  ...
  // ---------- dealer / staff figure ----------
  // opts.suit tints legs + vest (e.g. navy blazer for the receptionist);
  // hair stays near-black regardless so a colored suit doesn't dye the head.
  function makeDealer({ suit = '#1a1a1a', shirt = '#f2f0e8' } = {}) {
    const group = new THREE.Group();
    const blackMat = new THREE.MeshStandardMaterial({ color: suit, roughness: 0.6, metalness: 0.05 });
    const hairMat = new THREE.MeshStandardMaterial({ color: '#161616', roughness: 0.65, metalness: 0 });
    ...
    const hair = new THREE.Mesh(new THREE.SphereGeometry(0.116, 16, 12, 0, Math.PI * 2, 0, Math.PI / 2), hairMat);
    ...
    // Quick refusal gesture: three small head yaws. Instant no-op if REDUCED.
    group.userData.headShake = (app) => {
      if (app.REDUCED) return Promise.resolve();
      return new Promise((resolve) => {
        const t0 = performance.now(), dur = 650, base = headGroup.rotation.y;
        const hook = () => {
          const t = Math.min(1, (performance.now() - t0) / dur);
          headGroup.rotation.y = base + Math.sin(t * Math.PI * 5) * 0.16 * (1 - t);
          if (t >= 1) { headGroup.rotation.y = base; app.offFrame(hook); resolve(); }
        };
        hook.cancel = () => { app.offFrame(hook); resolve(); };
        app.onFrame(hook);
      });
