(() => {
  ...
  function buildWheel(scene) {
    ...
    const hub = new THREE.Mesh(new THREE.SphereGeometry(0.09, 20, 16), gold);
    hub.position.y = 0.2;
    hub.castShadow = true;
    group.add(hub);
    ...
    ring.position.y = 0.161;
    ring.receiveShadow = true;
    group.add(ring);

    const sepGeo = new THREE.BoxGeometry(0.22, 0.035, 0.018);
    for (let i = 0; i < 37; i++) {
      // pivot.rotation.y = -i*STEP places a box offset along local +X at
      // group-local angle +i*STEP AND orients its long axis (also local +X)
