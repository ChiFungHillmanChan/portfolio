// video-share-dialog.js — Per-hand replay video share. Opens INSIDE the
// replay modal when the user clicks Share on a specific hand. Records the
// live replay canvas via MediaRecorder, runs it through the daily video-
// share quota, and hands the blob to Web Share API (or downloads as
// fallback). Completely separate from share-dialog.js (graphs share) —
// renamed exports + DIALOG_ID prevent any cross-talk.
//
// Public API:
//   openVideoShareDialog({ host, getState, durationFn })
...
const DIALOG_ID = "replayVideoShareDialog";
...
export function openVideoShareDialog({ host, getState, durationFn }) {
  ensureDialog();
...
export function closeVideoShareDialog() {
  if (exporting) return;
