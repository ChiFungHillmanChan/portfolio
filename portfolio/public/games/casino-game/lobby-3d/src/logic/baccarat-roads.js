(() => {
  ...
  // Ambient-show cut card: placed shallow (72-96 cards ≈ 12-15 rounds) so
  // visitors actually witness the shuffle ritual — a realistic cut (~14 from
  // the end of 416) would take ~66 rounds of continuous watching to reach.
  const pickCutIndex = (rng = Math.random) => 72 + Math.floor(rng() * 25);
  ...
  C.baccaratRoads = { bacValue, total, bankerDraws, buildShoe, pickCutIndex, playRound, simulateShoe, stats, buildBigRoad, layoutRoad, bigRoadCells, beadPlate, deriveRoad, predictNext };
})();
