(() => {
  ...
  function makeImpl(app, built) {
    ...
    // AND is doing nothing but idle mocap (no activePath, no headGesture) has
    // its mixer.update() batched down to ~15Hz instead of every frame — dt
    // accumulates in lodAcc and is flushed as one lump update once it crosses
    // 1/15s (still exactly one mixer.update() call this function, same as
    // before this task — just not every call advances the mixer). The
    // instant an IK hand-path or a head-gesture one-shot goes active, this
    // falls back to full-rate unconditionally (checked fresh every frame, so
    // a path starting/ending while far immediately reflects in the very next
    // tick) — chopping a dealing arm or a nod/shake down to 15Hz reads as
    // visibly broken, per task-10-brief.md's caveat. Any leftover lodAcc is
    // folded into that frame's dt so no animation time is lost on the
    // throttled -> full-rate transition. castShadow toggling is independent
    // of the throttle (a far dealer's shadow is imperceptible whether or not
    // it happens to be mid-gesture that frame) and only costs a traverse on
    // the actual band-change frame.
