...
(function () {
    ...
    // Skip-100 ("simulate casino history") records spins via recordSpinOnly —
    // forward each one so the 3D tote board carries the same real records as
    // the 2D stats panel. Real bet spins are NOT forwarded here: they reach
    // the board when the 3D ball lands (the 'spin' message above).
    const origRecordSpinOnly = window.recordSpinOnly;
    window.recordSpinOnly = function (result) {
        const out = origRecordSpinOnly.apply(this, arguments);
        try {
            post({ type: 'stat-spin', result });
        } catch (err) {
            console.warn('[embed-3d] stat bridge failed:', err);
        }
        return out;
    };
