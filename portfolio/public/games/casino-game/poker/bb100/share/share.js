...
const state = {
  payload: null,
  shareId: null,
  // UI toggles. Default unit is $ — most viewers don't know what a BB
  // means without the stake, and the share dialog deliberately hides
  // the raw stake so $ is the most universal anchor.
...
function renderChart() {
  ...
  // Map the downsampled chart-index back onto the original "hand number" so
  // gridlines read "10,000 / 20,000" instead of the downsampled indices.
  ...
  // Detect series that are entirely zero (e.g. a snapshot generated by an
  // older compute pipeline that didn't populate showdown winnings). We hide
  // them from the legend rather than draw a misleading flat line at 0.
  const allZero = (arr) => !arr || arr.length === 0 || (
  ...
  state.chart = new Chart(document.getElementById("evChart").getContext("2d"), {
    ...
    options: {
      ...
      scales: {
        x: {
          // Replace Chart.js's evenly-spaced auto-ticks with hand-number-
          // aligned nice ticks (5,000 / 10,000 / 15,000…). Without this,
          // the downsampled labels produce ugly multiples like 3,777.
...
// Ported from upload.js — picks round-number x-axis ticks (5,000 / 10,000 /
// 15,000…) aligned to the original hand-number labels. Without it, the
// downsampled labels produce odd values like "3,777" / "7,547".
...
function pickNiceXTicks(labels, targetTicks = 7) {
  ...
  while (hand <= lastHand) {
    // Binary-search the chart index whose label is closest to this round
    // hand-number. We render the round number, not the off-by-N artifact.
