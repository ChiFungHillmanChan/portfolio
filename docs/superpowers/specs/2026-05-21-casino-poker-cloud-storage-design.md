...
### Precision bounds (per-100-hand checkpoint analysis, 2026-05-22 updated)

In addition to final-value matching, we verified cumulative green + orange at every 100 hands against the GGPoker app's chart readings (see `verify/verify-checkpoints.mjs` and `verify/checkpoint-experiment.mjs`). User re-read the GG app on 2026-05-22 and provided 19 checkpoints. Results vs current code:

- **Final orange ($11.23):** matches GG exactly
- **Final green ($8.71):** 1¢ under GG's $8.72 (sub-display-precision drift)
- **18 intermediate checkpoints (hand 100, 200, ..., 1800):** 8/19 match exactly; remainder drift up to ±6¢ in non-monotonic pattern
- **Total absolute drift across all checkpoints:** 35¢ (sum of per-checkpoint |Δ|)
...
We tested 7 alternative rake-share allocation formulas (2026-05-22):
| Strategy | Matches/19 | Σ|Δ| |
|---|---|---|
| A. current: (rake+jp) × heroWon / (pot − rake − cashDrop), round-half-up | **8** | **35¢** |
| B. same denom, truncate (no half-up) | 8 | 35¢ |
| C. denom = pot − cashDrop (rake stays in denom) | 0 | 91¢ |
| D. numerator = rake only (drop jackpot) | 0 | 803¢ |
| E. rake + jackpot computed via separate formulas | 8 | 35¢ |
| F. floor instead of half-up | 8 | 35¢ |
| G. denom = distributed pot (after fees + cashDrop) | 2 | 58¢ |

Current strategy A is the best of the 7. The residual is a real GGPoker-side algorithm difference (likely in split-pot / multi-way rake allocation) that we cannot replicate without GG's proprietary source.

**Precision contract:**
- bb/100 stat: accurate at all stakes
- Final cumulative: within 1¢ of GG over 1815 hands at NL2 (proportional ±$1 at NL200)
- Intermediate checkpoints: within ±6¢ at NL2 (proportional ±$6 at NL200)
