...
- Final green Winnings ≈ **+$8.50**
- Final orange All-in EV ≈ **+$11.00**
...
**Phase 1's verify CLI is a HARD GATE.** No Phase 2+ work begins until the CLI prints `Green: $8.50  Orange: $11.00` exactly. Same rule as the old E spec.
...
1. **Verify CLI HARD GATE** — run `node verify/verify.mjs` against the 1815-hand sample; confirm green=$8.50 and orange=$11.00 to the cent, run twice for bit-identical output. If numbers don't match, fix before Phase 2.
...
Final Winnings (before rake):   $8.XXXXXX     ← rounds to $8.50 (must match GG)
Final All-in EV (after rake):   $X.XXXXXX
Final All-in EV (before rake):  $11.XXXXXX    ← rounds to $11.00 (must match GG)
...
**Phase 1 done = exact match.** When rounded to 2 decimals, green = $8.50 and orange = $11.00. No tolerance. Run twice — every digit identical. If numbers don't match, fix before Phase 2.
...
Phase 1 — Local foundation (Sub-project E)         ~1 day remaining (mostly done)
  ...
  Remaining: run verify CLI → confirm $8.50 / $11.00 to the cent (HARD GATE);
