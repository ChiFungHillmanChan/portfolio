...
- **Tests run via** `node --test js/wallet/*.test.mjs` from the `calculator/` dir. (Use the GLOB form, not the bare directory `node --test js/wallet/` — because the dev-only `js/wallet/package.json` below has no `main`, Node resolves the bare directory as a module entry and fails with MODULE_NOT_FOUND. The glob targets the test files directly.) A dev-only `js/wallet/package.json` = `{"type":"module"}` makes Node treat the `.js` modules as ESM (mirrors `ultimate-texas-holdem/package.json`, whose package.json sits in the parent, not the tests dir). That file is inert in browsers.
